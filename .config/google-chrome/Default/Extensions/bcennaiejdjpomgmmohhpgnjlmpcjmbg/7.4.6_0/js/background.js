...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) {
	...
	var isCtrlZEnabled = bg.settings.ctrlZ;
	if (request == "isCtrlZEnabled") {
		sendResponse(isCtrlZEnabled);
...
chrome.webNavigation.onTabReplaced.addListener(function (details) {
	...
	chrome.tabs.get(details.tabId, function (tab) {
		...
		forgetOpenTab(details.replacedTabId);
	});
});
...
if (localStorage.tablist_cleanup_done != 'true') {
  localStorage.tablist_cleanup_done = 'true';
	...
	// add currently opened tabs back
