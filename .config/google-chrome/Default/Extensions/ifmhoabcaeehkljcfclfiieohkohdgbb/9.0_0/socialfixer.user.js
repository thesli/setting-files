...
var undef;
var config_url = "http://socialfixer.com/config9.json?type="+SCRIPT_TYPE+"&version="+version;
	...
	setValue = function(key,val,func) {
		try {
			chrome.extension.sendRequest( {"command":"setvalue", "key":key, "value":val }, function(ret) {
				...
				var storySelector = ".uiUnifiedStory,.timelineUnitContainer,.mall_post,div[data-dedupekey]";
						...
						function better_fb_options(tab_id) {
							...
							bind(document,'mouseover',function(e) {
								...
								if (tn && (tn=="IMG" || tn=="I" || tn=="A") ||tn=="DIV") {
									...
									if (tn=="A" || tn=="DIV") {
										if (hasClass(image_preview_target,/(^|\s)(uiPhotoThumb|photoWrap|uiScaledImageContainer|_5pc0)(\s|$)/)) {
...
//							});
						...
						execute_in_page_scope( {'bind':bind,'click':click,'append':append}, function() {
							unsafeWindow.requireLazy(["LitestandStreamUnloader","LitestandStreamLoader"],function(LitestandStreamUnloader,LitestandStreamLoader) { 
									...
									if (unsafeWindow && unsafeWindow.requireLazy) {
										...
										var init = function() {
											...
											setInterval(function() {
												if (UIIntentionalStream && UIIntentionalStream.instance && UIIntentionalStream.instance.root && !UIIntentionalStream.instance.root.sfx_attached) {
													init();
												}
											},1000);
										});
