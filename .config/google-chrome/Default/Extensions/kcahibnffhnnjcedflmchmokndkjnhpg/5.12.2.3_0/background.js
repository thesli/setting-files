...
var BackgroundPage = Class.extend({
    ...
    */
    ...
    onGetContests: function(tabid)
    {
        var contests = Contests.getAll();
        this.sendApiMessage(tabid, "msgContestsResponse", contests);
    },
    ...
    handleApiMessage: function(msg, sender)
    {
        ...
        case "msgGetContests":
            // onGetContests(tabid);
            break;
        case "msgActivateContest":
            Contests.activate(msg.data);
            break;
        case "msgResetContest":
            Contests.resetNotifications(msg.data);
            break;
    ...
    */
    ...
    onStartSession: function(request, sender, fnResponse) {
        // Make sure to only attempt a session refresh every once in a while, this will avoid
        // an infinite loop when cookies are disabled or there are site problems
        var now = (new Date()).getTime();
        if (!request.force) {
            var lastRefresh = SU.Storage.get("lastSessionRefresh");
            if (lastRefresh) {
                lastRefresh = parseInt(lastRefresh);
                if (now - lastRefresh < this.SESS_PAUSE_REFRESH) {
                    fnResponse(false);
                    return;
                }
            }
        }
        SU.Storage.set("lastSessionRefresh", now);

        // Make sure the URL matches our server
        var ourServer = Strings.get("SERVER");
        var regex = new RegExp('^https?:\\/\\/' + ourServer + '($|\\/)');
        if (!request.url.match(regex)) {
            fnResponse(false);
            return;
        }

        // Let them know that we are refreshing
        fnResponse(true);

        var api = getClientApi();
        var sessionUrl = api.getStartSessionUrl(request.url);
        sender.tab.url = sessionUrl;
    },
    ...
    onRequest: function(request, sender, fnResponse)
    {
        ...
        switch(request.type)
        {
            ...
            case "REFRESH_STATE":
                ...
                break;
            case "STARTSESSION":
                this.onStartSession(request, sender, fnResponse);
                break;
            case "GET_URLDATA":
    ...
    updateBadgeIcon: function(tab) {
        ...
        // Even if blacklisted, contest URLs don't get the warning icon
        if(window.Contests && Contests.isContestUrl(tab.url))
            return;
