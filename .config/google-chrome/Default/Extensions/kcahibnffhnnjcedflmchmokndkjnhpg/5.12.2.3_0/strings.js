...
var Strings = {
    ...
    FIRST_RUN: "@@API20_HTTPS@@/client/firstrun?x_su_version=@@ENC_XSUVERSION@@&client_type=@@DEVICE@@&clientid=@@CLIENTID@@&prev_version=@@PREV_VERSION@@&x_su_consumer_key=@@CONSUMER_KEY@@&x_su_access_token_key=@@ACCESS_TOKEN@@",
    ...
    STUMBLE_URL: "@@SERVER_HTTPS@@/to/stumble/go/" + "?device=@@DEVICE@@&version=@@VERSION@@&clientid=@@CLIENTID@@&x_su_consumer_key=@@CONSUMER_KEY@@&x_su_access_token_key=@@ACCESS_TOKEN@@",
    ...
    CONTESTS_URL: "@@SERVER_HTTP@@/su/contests/contests",
    ...
    get: function(name, params) {
        ...
        switch(name) {
            ...
            case "SERVER_HTTP":
            case "SERVER_HTTPS":
                // We use tokens with nearly every request, so always use HTTPS so we don't send tokens unencrypted
                if (this.isDevDomain())
                    str = "http://@@SERVER@@";
                else
                    str = "https://@@SERVER@@";
