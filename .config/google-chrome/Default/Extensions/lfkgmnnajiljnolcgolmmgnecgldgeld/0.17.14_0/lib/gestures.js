var DEBUG = false;
d = function(msg) { if (DEBUG) console.debug(msg); };
dd = function(obj) { if (DEBUG) console.dir(obj); };
jj = function(obj) { return JSON.stringify(obj); };
...
if(location.hostname == "www.google.com" && location.hash.substr(0,5) == "#:--:" && !window.SGHistory) {
  ...
  chrome.extension.sendMessage({loadhistory:index,id:id}, function(resp) {
    if(resp) {
...
///////////////////////////////////////////////////////////
  _this.connect = function() {
    ...
    if(window.SGextId) {
      d('connecting1 ' + window.SGextId);
      port = chrome.extension.connect(window.SGextId, mess);
    }
    else {
      d('connecting2 ' + jj(mess));
      port = chrome.extension.connect(mess);
    }
    if(!port) return;
  ...
  var receiveMessage = function(mess) {
    ...
    if(mess.chain) {
      startGesture(mess.chain.startPoint, 
                   (mess.chain.startPoint ? document.elementFromPoint(mess.chain.startPoint.x,mess.chain.startPoint.y) : null), 
                   mess.chain.line, mess.chain.rocker, mess.chain.wheel, 10000); //1500);
      blockContext = true;
...
// end SmoothGestures object

d('SGinjectscript: ' + window.SGinjectscript);
...
d('window.SG: '  + window.SG);
if(window.SG) {
  d('window.SG: '+ window.SG.enabled());
