...
var get_premium_html =
    ...
    +'<img src="/img/gift.png">'+T('Get Free Premium')+'</a></li>';
var sign_up_html =
...
E.www_user_links = E.user_links.extend({
    initialize: function(options){
        this.options = _.defaults(options||{}, {premium_link: true});
...
E.be_user_links = E.user_links.extend({
    initialize: function(options){
        ...
        this.listenTo(this.model, 'change', this.render);
        this.prot = 'https:';
        if (zutil.version_cmp(be_util.version(), '1.2.672')>=0)
    ...
    render: function(){
        ...
        this.$el.empty();
        /* XXX amir: find a better way to link to https pages */
	var settings = zutil.version_cmp(be_util.version(), '1.2.715')>=0 ?
        ...
        if (this.options.upgrade)
        {
            $ul.append(!is_member ? '<li><a class="user_link" '
                +'href="https://hola.org/premium.html?'
                +'ref=be_user_nav">Upgrade</a></li>' : '');
        }
        this.add_lang($ul);
        if (zutil.version_cmp(be_util.version(), '1.2.661')>=0)
        {
