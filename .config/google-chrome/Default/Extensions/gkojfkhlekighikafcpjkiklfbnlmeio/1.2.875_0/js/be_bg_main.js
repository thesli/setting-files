...
define(['jquery', 'underscore', 'be_backbone', 'etask', 'be_util', 'be_zerr',
    'be_browser', 'be_lib', 'escape', 'svc_util', 'string',
    'be_config', 'bootstrap'],
    function($, _, be_backbone, etask, be_util, zerr, B, be_lib, zescape,
    zutil, string, be_config, bootstrap){
B.assert_bg('be_bg_main');
be_lib.set_TypeError_handler();
...
E.on_ccgi_url_change = _.debounce(function(){
    var qs = {browser: E.get('browser'), ver: be_util.version(),
        plugin: +E.get('exe.is_plugin')||undefined};
    if (E.get('exe.connected'))
	qs.cid = E.get('cid');
    else
	qs.uuid = E.get('uuid');
...
E.init = function(install_details){
    ...
    E.on_init('change:browser change:cid change:ccgi_url_base change:uuid '+
        'change:use_http change:exe.is_plugin', E.on_ccgi_url_change);
...
E.check_svc = function(){
    return etask([function(){ return be_util.svc_callback();
        ...
        if (r.status && r.status.unblocker)
        {
            ...
            change['status.unblocker.connected'] = r.plugin ?
                r.status.unblocker.enabled : r.status.unblocker.connected;
	...
	/* XXX bahaa: need to rm status.* once svc is not found anymore */
        ...
        E.safe_set({'svc.info': null, ws_port: 0,
            ...
            'status.unblocker.pac_url': null,
            ccgi_url_base: null, cid: 0, session_key_cid: 0,
            'svc.running': false,
        });
    }]);
};
