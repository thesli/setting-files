...
define(['jquery', 'etask', 'be_zerr', 'be_msg', 'be_backbone',
    ...
    'svc_util'],
    function($, etask, zerr, be_msg, be_backbone, be_chrome, _, string, zutil){
...
{
    var have = {}, ver = zconf.ZON_VERSION;
    if (zutil.version_cmp(ver, '1.1.892')>=0)
        have['be.mem_info'] = true;
    if (zutil.version_cmp(ver, '1.1.678')<0)
        have['be.proxy.refresh'] = true;
    if (zutil.version_cmp(ver, '1.1.626')<0)
        have['be.proxy.set_pac_options'] = true;
    else if (zutil.version_cmp(ver, '1.1.762')<0)
        have['be.proxy.set_rules'] = true;
    if (zutil.version_cmp(ver, '1.2.600')>=0)
        have['firefox.panel.close'] = true;
    if (zutil.version_cmp(ver, '1.2.113')>=0)
        have['runtime.request_update_check'] = true;
    if (zutil.version_cmp(ver, '1.1.759')>=0)
        have.req_listeners = true;
    if (zutil.version_cmp(ver, '1.1.737')>=0)
        have['runtime.set_uninstall_url'] = true;
    return have;
}
...
E.windows = {
    ...
    WINDOW_ID_NONE: chrome ? chrome.windows.WINDOW_ID_NONE : null,
};
E.storage = {
    local: {
...
E.be = {
	...
	on_auth_required: {
            set_listener: function(cb_name, cb){
                E.rpc.call_api('be.proxy.auth_required', '',
                    'set_listener', [cb_name], cb);
            },
            clear_listener: function(cb){
                E.rpc.call_api('be.proxy.auth_required', '',
