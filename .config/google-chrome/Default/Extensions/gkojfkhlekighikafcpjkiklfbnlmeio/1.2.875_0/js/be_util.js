...
var firefox_id = 'jid1-4p0kohsjxu1qgg-at-jetpack';
...
E.old_unsupported_ext = function(){
    return zutil.version_cmp(E.version(), '1.1.563')<=0; };

E.old_ext = function(){
    return zutil.version_cmp(E.version(), '1.1.809')<=0; };
...
E.svc_callback = function(opt){
	...
	if (!E.is_plugin() && !is_svc)
        ...
        {
            ret.ccgi_url_base =
                data.cgi_link.replace(/^http:\/\//, 'https://');
        }
...
E.svc_version_check = function(version, be_ext){
    var ver = be_ext.get('exe.version')||'';
...
E.open_be_tab = function(opt){
    ...
    _opt.tab_match =
        chrome ? {url: 'chrome-extension://'+chrome.runtime.id+'/*'} :
	{url_re: '^resource:\\/\\/'+firefox_id};
    _opt.url = (chrome ? 'js/' :
        'resource://'+firefox_id+'/hola_firefox_ext/data/')+_opt.url;
    return E.open_tab(_opt);
};
