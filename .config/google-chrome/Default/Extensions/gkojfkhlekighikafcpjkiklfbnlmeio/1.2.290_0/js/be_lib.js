...
E.perr = function(level, opt){
    ...
    var filehead_type = opt.filehead_type, skip_ver_check = opt.skip_ver_check;
	...
	filehead_type: filehead_type, rate_limit: opt.rate_limit};
    if (RMT)
	qs = RMT.auth();
    else if (bg_main)
    {
...
E.open_ccgi = function open_ccgi(opt){
    function create_tab(url){ B.tabs.create({url: url}); }
    /* XXX arik/mark: check encoding of hash */
    var url = opt.ccgi_url+(opt.host ? '&url='+encodeURIComponent(opt.host) :
	'')+(opt.hash ? '#'+opt.hash : '');
    /* XXX arik: wrap with etask */
    B.tabs.query(_.extend({lastFocusedWindow: true}, chrome ?
        {url: conf.ccgi_match} : {url_re: conf.ccgi_re}),
	function(tabs){
	if (!tabs || !tabs.length)
