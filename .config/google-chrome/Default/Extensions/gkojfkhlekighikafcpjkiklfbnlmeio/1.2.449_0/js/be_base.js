...
E.to_uri_comp = function to_uri_comp(s)
{
    return encodeURIComponent(s).replace(/%20/g, '+');
};

E.to_uri = function to_uri(param, opt)
{
    var uri = '';
    if (!param)
        return uri;
    opt = opt||{};
    var to_uri_comp = opt.space_plus ? E.to_uri_comp :
	encodeURIComponent;
    for (var i in param)
    {
	var val = param[i];
...
{
    var conf = E.get_conf();
    var url = (conf ? conf.url_ccgi : 'https://client.E.org/client_cgi')+
	'/perr';
    var xhr = new XMLHttpRequest();
    xhr.open('POST', url+'?'+E.to_uri(opt, {space_plus: true}));
    xhr.send(null);
};
