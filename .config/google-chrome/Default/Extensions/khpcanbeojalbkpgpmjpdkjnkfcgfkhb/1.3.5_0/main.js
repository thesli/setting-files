...
var yass = {
	...
	handleEvent: function(ev)
	{
		...
		var speed =  Math.min(300, evtime * (this.m_pref.tickwheel / Math.abs(ev_detail)));
		...
		else 
		{
			...
			if(fromkeyboard) {
				...
				// accel damper
				var timef = Math.max(0, ((1000.0 - (ctm - this.m_lasteventtime)) / 1000.0)); // 1.0 -> 0.0 in 1000ms
				var lastaccel = this.m_lastaccel * timef;
				if(this.m_lastd * ev_detail < 0) lastaccel = 1.0;
				var lastratio = (lastaccel < accel) ? 0.25 : 0.75;
				accel = (accel * (1.0 - lastratio)) + (lastaccel * lastratio);
				this.m_lastaccel = accel;
			}
			var delta = step*((ev_detail<0)?-1:1);
