...
function initContestMenuItems(url, loggedIn) {
    var items = gBackgroundPage.Contests.getMenuItems(url);
    if(!items || !items.length)
        return;

    // No disabled message when contest is active
    document.getElementById("disabledMessage").style.display = "none";
    var contestLinks = document.getElementById("contestLinks");
    contestLinks.style.display = "block";
    var ol = document.getElementById("contestButtons");
    var firstChild = ol.firstChild;
    for(var i=0; i<items.length; i++) {
        if(!loggedIn && (items[i].loggedOutBehavior == "hidden"))
            continue;
        ...
        a.href = items[i].href;
        // Set up the image if there is one
        if(items[i].icon) {
            a.style.backgroundImage = "url(" + items[i].icon + ")";
            a.className = "bgImgLeft";
        }

        if(loggedIn || (items[i].loggedOutBehavior != 'disable'))
        {
            // Need a closure for the click handler
            var setupAnchorClick = function(a, url, newTab) {
                a.addEventListener("click", function() {
                    gBackgroundPage.Utils.navigate(url, newTab);
                    window.close();
                    return false;
                }, false);
            }
            setupAnchorClick(a, items[i].href, items[i].newTab);
            li.setAttribute("class", "btn");
        }
        else
        {
            li.style.opacity = "0.5";
            li.setAttribute("class", "btn btnDisabled");
            a.setAttribute("title", "Must be signed in");
        }
        ...
        // Set the item styles
        for(var name in items[i].css)
            li.style[name] = items[i].css[name];
        for(var name in items[i].cssAnchor)
            a.style[name] = items[i].cssAnchor[name];
...
function init(loggedIn) {
    ...
    chrome.tabs.getSelected(null, function(currentTab) {
        ...
        initContestMenuItems(currentTab.url, loggedIn);
