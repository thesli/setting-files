Kotlin.defineModule('chrome-ext', ['browser-ext-platform', 'jb-chrome-ext-api', 'chrome-ext-options'], function () {
  ...
  Kotlin.p(_, 'com_jetbrains_browserConnection_chrome', function () {
    ...
    function $f3(rpcServer, tab, it) {
      rpcServer.send("Debugger", "inspect", JSON.stringify((tab !== null && tab !== undefined ? tab : Kotlin.throwNPE()).url) + ", " + it);
    }
    function $f4(it) {
    }
    function $f5(pageManager, rpcServer, data, tab) {
      ...
      if (Kotlin.equals((tmp$0 = data.menuItemId) !== null && tmp$0 !== undefined ? tmp$0 : Kotlin.throwNPE(), "inspect")) {
        var tmp$1;
        (tmp$1 = pageManager.v) !== null && tmp$1 !== undefined ? tmp$1.attachDebugger(tab !== null && tab !== undefined ? tab : Kotlin.throwNPE(), true, $f3.bind(null, rpcServer, tab), $f4) : null;
      }
    }
    function $f6() {
    ...
    function $f8(disposable, socket, pageManager, buildInfo, rpcServer, host, port, connected) {
      try {
        disposable.register($f0.bind(null, socket));
        pageManager.v = _.com_jetbrains_browserConnection_chrome.ChromePageManager(buildInfo, rpcServer);
        var tmp$0, tmp$1;
        ...
        if (Kotlin.modules['browser-ext-platform'].com_jetbrains_browserConnection.greaterOrEquals(buildInfo, 134, 312)) {
          Kotlin.modules['jb-chrome-ext-api'].org_jetbrains_chrome.clicked$0(chrome.contextMenus, disposable, $f5.bind(null, pageManager, rpcServer));
          if (_.com_jetbrains_browserConnection_chrome.registeredContextMenuItems.length === 0) {
            var tmp$2;
            _.com_jetbrains_browserConnection_chrome.registeredContextMenuItems.push(chrome.contextMenus.create({id: "inspect", title: "Inspect in " + ((tmp$2 = buildInfo.productName) !== null && tmp$2 !== undefined ? tmp$2 : buildInfo.name), documentUrlPatterns: ["http://*/*", "https://*/*"]}, $f6));
          }
           else {
    ...
    function $fa(disposable, pageManager, buildInfo, host, port, connected, socket, rpcServer) {
      socket.opened = $f8.bind(this, disposable, socket, pageManager, buildInfo, rpcServer, host, port, connected);
    ...
    Object.defineProperty(this, 'connect', {value: function (buildInfo, host, port, connected, disposable) {
      ...
      Kotlin.modules['browser-ext-platform'].com_jetbrains_browserConnection.connect(chrome.app.getDetails().version, window.navigator.userAgent.indexOf("(Dart)") !== -1 ? "DARTIUM" : "CHROME", host, port, -1, $fa.bind(this, disposable, pageManager, buildInfo, host, port, connected));
    }});
