...
var extId = details.id;
var extVersion = details.version;

var log = {action:{}};
var nav = navigator.platform.match(/Win/) ? "win" :
      (navigator.platform.match(/Mac/) ? "mac" :
       (navigator.platform.match(/Linux/) ? "linux" :
        "unknown"));
...
var DEBUG = false;
d = function() { if (DEBUG) console.log.apply(console, arguments); };
dd = function(obj) { if (DEBUG) console.dir(obj); };
jj = function(obj) { if (DEBUG) return JSON.stringify(obj); return ''; };
...
var pageAction = function(tabId, icon, title) {
  try {
    chrome.pageAction.setIcon({tabId: tabId, path: chrome.extension.getURL(icon)});
...
var actions = {
  'new-tab': function(id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function(tab) {
      var prop = {
        url:settings.newTabUrl!="homepage"?settings.newTabUrl:undefined, windowId:tab.windowId};
  ...
  'undo-close-hack': function(id, call) {
    ...
    for(var i=start; i<t.history.length-1; i++)
      if(t.history[i].substr(0,4) != "http")
        start = i;
    ...
    if(t)
      chrome.windows.get(t.winId, function(win) {
        if(t.history.length > 1) {
          ...
          var url = "http://www.google.com/?index=0#:--:"+r+":--:"+escape(JSON.stringify(t.titles))
  ...
  'page-back': function(id, call) {
    runJS(id, "history.back()"); call();
  },
  'page-forward': function(id, call) {
    runJS(id, "history.forward()"); call();
  },
  'new-window': function(id, call) {
    chrome.windows.create({url:settings.newTabUrl!="homepage"?settings.newTabUrl:undefined}, call);
  },
  ...
  'close-window': function(id, call) {
    chrome.windows.getCurrent(function(win) { chrome.windows.remove(win.id, call); });
  },
        ...
        chrome.tabs.getAllInWindow(win.id, function(tabs) {
          chrome.windows.create({incognito:win.incognito}, function(newwin) {
            for(i=tab.index; i<tabs.length; i++) {
              chrome.tabs.move(tabs[i].id, {windowId:newwin.id, index:i-tab.index});
            }
  ...
  'options': function(id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function(tab) {
      chrome.tabs.create({url:chrome.extension.getURL("/views/options.html"), windowId:tab.windowId}, call);
    });
  },
  'status': function(id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function(tab) {
      chrome.tabs.create({url:chrome.extension.getURL("/views/status.html"), windowId:tab.windowId}, call);
    });
  },
  'page-back-close': function(id, call) {
    runJS(id, "history.back();"+(activeTabs[contents[id].detail.tabId].history.length==1?" setTimeout(function(){ port.postMessage(JSON.stringify({closetab:true})); },400);":"")); call();
  },
    ...
    chrome.windows.getLastFocused(function(win) {
      if(!maximizeXWindow[win.id]) {
        ...
        chrome.windows.update(win.id, {
          ...
          top:dim.top,
          width:dim.width,
          height:dim.height}, function() {
            setTimeout(function() {
              chrome.windows.update(win.id, {
                left:dim.left,
                top:dim.top,
                width:dim.width,
                height:dim.height}, call);
            }, 10);
          });
      }
    });
            ...
            chrome.windows.update(mwin.id, {
              ...
              height:10}, function() {
                ...
                if(i < 0)
                  ...
                  focusWindow(focusedWindows[i], call);
              });
          }, 10);
        });
    });
  ...
  'parent-dir': function(id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function(tab) {
      ...
      if(parts.length > 3)
        ...
        url = parts.join("/")+"/";
      if(url)
        chrome.tabs.update(tab.id, {url:url}, call);
      else call();
    });
  },
  ...
  'pin': function(id, call) {
    chrome.tabs.get(contents[id].detail.tabId, function(tab) {
      if(chromeVersion >= 9)
        chrome.tabs.update(tab.id, {pinned:true}, call);
    });
  },
    ...
    chrome.tabs.get(contents[id].detail.tabId, function(tab) {
      chrome.bookmarks.search(tab.url, function(bmks) {
        if(bmks.length <= 0)
          ...
          chrome.bookmarks.remove(bmks[0].id, call);
      });
    });
  },
    ...
    chrome.tabs.getAllInWindow(winId, function(tabs) {
      (minimizeXWindow[winId] ? function(winId, setDim) {
        if(minimizeXWindow[winId].listener)
          chrome.windows.onFocusChanged.removeListener(minimizeXWindow[winId].listener);
...
// External ///////////////////////////////////////////////
///////////////////////////////////////////////////////////
PLUGIN_08_LOCAL_ID = "apagmdofhjomjncpiebpaaonngppcpcl";
PLUGIN_09_LOCAL_ID = "igagkbmimnkligicolgbkglpbldomdig";
PLUGIN_09_WEBSTORE_ID = "dfjpomofbadillhmdjcjfbbdghgikmac";

chrome.extension.onConnectExternal.addListener(function(port) {
  d('chrome.extesion.onConnectExternal...');
  if (port.name == "sgplugin"
      && (port.sender.id == PLUGIN_09_LOCAL_ID
          || port.sender.id == PLUGIN_09_WEBSTORE_ID
          || port.sender.id == PLUGIN_08_LOCAL_ID)) {
    d('incoming connection from sgplugin...');
    pluginport = port;
...
chrome.extension.onMessageExternal.addListener(function(request, sender, respond) {
  if(request.getgestures) {
    if(gesturesjs)
      respond({gestures:gesturesjs});
    else {
      $.get(chrome.extension.getURL("/lib/gestures.js"), null, function(data) {
    ...
    if(ex.name && ex.actions) {
      if(ex.actions.length > 0) {
        ...
        delete externalActions[sender.id];
      }
      saveExternalActions();
      respond(true);
    }
...
var pluginConnected = function() {
  return pluginport != null;
};

// Handle Gesture /////////////////////////////////////////
///////////////////////////////////////////////////////////
var contentMessage = function(id, mess) {
  d('BEGIN contentMessage: ' + jj(mess));
  ...
  if (mess.gesture && gestures[mess.gesture]) {
    ...
    if (mess.gesture[0]=="r")
      chainGesture = {
        rocker:true,
        timeout:setTimeout(function(){chainGesture=null;},2000)};
    if (mess.gesture[0]=="w")
      chainGesture = {
        wheel:true,
        timeout:setTimeout(function(){chainGesture=null;},2000)};
    ...
    try {
      if(actions[gestures[mess.gesture]])
        ...
        chrome.extension.sendMessage(gestures[mess.gesture].substr(0,32), {doaction:gestures[mess.gesture].substr(33)});
      else
  ...
  if(mess.alertDoubleclick) {
    ...
    window.rightclickPopup = window.open("http://www.smoothgestures.com/installplugin.html", "rightclick", "width="+width+",height="+height+",top="+wtop+",left="+wleft);
  }
  if(mess.closetab) {
    if(mess.closetab == true)
      mess.closetab = contents[id].detail.tabId;
...
var initConnectTab = function(port) {
  d('initConnectTab: ' + jj(port.sender) + '/' + port.sender.tab + '/' + port.detail.id);
...
var updateActiveTab = function(tabId, change) {
  chrome.tabs.get(tabId, function(tab) {
    if(change && change.url) {
      tab.url = change.url; tab.title = change.url;
    }
    ...
    if(tab.status == "loading" && !settings.hidePageAction) {
      pageAction(tabId, "/assets/img/pageaction.png", "Smooth Gestures")
    }
...
chrome.tabs.onRemoved.addListener(function(tabId) {
  if(activeTabs[tabId])
    closedTabs.push(activeTabs[tabId]);
...
var getTabStatus = function(tabId, callback) {
  ...
  if(!content) {
    chrome.tabs.get(tabId, function(tab) {
      if(!tab)
        callback("broken");//fix for "no tab with id:"
...
var refreshPageAction = function(tabId) {
  getTabStatus(tabId, function(stat) {
    if(stat == "unable") {
      pageAction(tabId, "/assets/img/pageaction-unable.png", "Chrome blocks Gestures on this page")
    } else if(stat == "broken") {
      pageAction(tabId, "/assets/img/pageaction-broken.png", "Gestures don't work. Reload")
...
var loadOptions = function(profile, call) {
  ...
  if(options) {
    ...
    for(x in options.settings)
      settings[x] = options.settings[x];
  }
  updateValidGestures();
...
var initialize = function() {
  ...
  var firstRun = !getValue('id')
        && !getValue('date_first')
        && !getValue('initialized_before');

  if(!getValue('id'))
    setValue('id', instanceId);
  if(!getValue('profile'))
    setValue('profile', profile);
  if(!getValue('date_first'))
    setValue('date_first', (new Date()).toString());

  //detect and update version updates
  var newVersion = [extVersion, getValue('version')].sort()[0] != extVersion;
  if(newVersion)
    setValue('date_updated', (new Date()).toString());
  setValue('version', extVersion);

  //connect to sgplugin
  d('BEGIN sendMessage to plugins');
  chrome.extension.sendMessage(PLUGIN_08_LOCAL_ID, {connectplugin:true});
  chrome.extension.sendMessage(PLUGIN_09_LOCAL_ID, {connectplugin:true});
  chrome.extension.sendMessage(PLUGIN_09_WEBSTORE_ID, {connectplugin:true});
  d('END sending to plugins');

  //refresh external actions
  for(id in externalActions) {
    delete externalActions[id];
    chrome.extension.sendMessage(id, {getexternalactions:true});
  }
  saveExternalActions();

  //reconnect to any tabs that already run gestures.js
    ...
    getTabStates(function(states) {
      ...
      for(winId in states)
        for(i=0; i<states[winId].length; i++)
          if(states[winId][i].root)
            connectedtab = true;
      else if(!states[winId][i].goodurl)
        blockedtab = true;
      else if(states[winId][i].tabStatus != "loading")
        brokentab = true;
      //UPDATE FIXES
      chrome.bookmarks.getTree(function(tree) {
        var other = tree[0].children[1];
      for(var i=0; i<other.children.length; i++)
        if((other.children[i].title == "GestureSync" || other.children[i].title == "Smooth Gestures Sync"))//&& other.children[i].children)
    ...
    for(x in wins) {
      ...
      for(y in wins[x].tabs) {
        (function(tab) {
          ...
          if(!tab.url.match(/(^chrome(|-devtools|-extension):\/\/)|(:\/\/chrome.google.com\/)|(^view-source:)/)) {
            chrome.tabs.executeScript(tab.id, {
              allFrames:true,
              code: "if(window.SG) { if(window.SG.enabled()) window.SG.disable(); delete window.SG; }"});
            setTimeout(function() {
              chrome.tabs.executeScript(tab.id, {allFrames:true, file:"/lib/gestures.js"});
            },200);
          }
          setTimeout(function() {
            refreshPageAction(tab.id);
...
var migrateSettings = function() {
  d('BEGIN migrateSettings ' + localStorage.length);
  ...
  d('END migrateSettings');
};

d('AU.init...');
AU.init({
  donateUrl: 'http://www.smoothgestures.com/donate',
  postInstallUrl: 'http://www.smoothgestures.com/thankyou',
  updateUrl: 'http://u.yieldsquare.com/smoothgestures.json',
  upgradeUrl: 'http://www.smoothgestures.com/donate/#upgrade',
  gaid: 'UA-45074427-1',
  sfid: 'tduqbwo'
}, function() {
  /////////////////////////////////////////////////////////////////////////////
  d('AU.storage...');
  AU.storage(function(_storage) {
    storage = _storage;

    d('storage callback begin...');
    ...
    // Communication //////////////////////////////////////////
    ///////////////////////////////////////////////////////////
    d('setting up onMessage listener');
    chrome.extension.onMessage.addListener(function(request, sender, respond) {
      d('chrome.extension.onMessage.addListener: ' + jj(request) + ' / ' + jj(sender));
      if(request.getstates) {
        ...
        case 'saveLocalStorage':
          for (var key in request.data) {
            storage.set(key,request.data[key]);
          }
          storage.set('importedFromForeground',true);
          respond(storage);
          break;
        case 'localStorage':
          switch (request.operation) {
          case 'getItem':
            respond({status: true, value: storage.get(request.itemName)});
            break;
          case 'removeItem':
            storage.remove(request.itemName);
            respond({status: true, value: null});
            break;
          case 'setItem':
            storage.set(request.itemName, request.itemValue);
            respond({status: true, value: null});
            break;
          }
          break;
        ...
        if(request.reloadtab == true)
          request.reloadtab = sender.tab.id;
        chrome.tabs.get(request.reloadtab, function(tab) {
          chrome.tabs.update(tab.id, {url:tab.url}, respond);
        });
      } else if (request.rightclick == 'doubleclick') {
        d('rightclick message received');
        settings.blockDoubleclickAlert = true;
        saveOptions(profile);
        respond(true);
      } else {
        d('unrecognized request: ' + jj(request));
      }
    });

    d('chrome.extension.onConnect.addListener...');
    chrome.extension.onConnect.addListener(function(port) {
      d('chrome.extension.onConnect: ' + port.name + '/' + jj(port.sender.tab));
