...
function initfader(src){
	...
	if(id.match(/^.*_w$/)){
		if(id == "step_w") {
			src.onchange = function(){faderonscroll(src);updategraph("w");faderonscrollstepw();};
		} else {
			src.onchange = function(){faderonscroll(src);updategraph("w");};
		}
	} else if(id.match(/^.*_k$/)){
		src.onchange = function(){faderonscroll(src);updategraph("k");};
	} else {
		src.onchange = function(){faderonscroll(src);};
	}
